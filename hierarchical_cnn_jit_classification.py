...
class DeepJITExtented(nn.Module):
    def __init__(self, args):
        super(DeepJITExtented, self).__init__()
        self.args = args
        ...
        self.fc1 = nn.Linear(self.args.n_file * 2 * len(Ks) * Co + args.extended_ftr + len(Ks) * Co,
                             args.hidden_size)  # hidden units
    ...
    def forward_code(self, x, convs_line, convs_hunks):
        n_batch, n_file, n_line = x.shape[0], x.shape[1], x.shape[2]
        x = x.reshape(n_batch * n_file * n_line, x.shape[3], x.shape[4])
        ...
        x = x.reshape(n_batch * n_file, n_line, self.args.num_filters * len(self.args.filter_sizes))
    ...
    def forward(self, ftr, msg, added_code, removed_code):
        ...
        x_added_code = self.forward_code(x_added_code, self.convs_code_line, self.convs_code_file)

        x_removed_code = self.embed_code(removed_code)
        x_removed_code = self.forward_code(x_removed_code, self.convs_code_line, self.convs_code_file)
